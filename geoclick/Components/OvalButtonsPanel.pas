...
begin
  ...
  for i := 1 to FNButtons do begin
    Buttons[i].Caption := Copy(VL.Strings[i-1],1,(ButtonWidth div Font.Size)+2);
...
begin
  if CurrenTOvalButton <> nil then begin
    ...
    if Disappear then begin
      CurrenTOvalButton.Free;
      Buttons[curr] := nil;

    end else

      CurrenTOvalButton.Color := FDisableColor;
  ...
  for AlrGen := 1 to NQuestions do begin
    ...
    repeat
      ...
      if FValueList.IndexOf(strpr) = -1 then begin
       ...
       // FValueList.Add(Copy(strpr,1,(ButtonWidth div Font.Size)+2));
        Added := True;
      end;
    until Added;
