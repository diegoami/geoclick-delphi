...
uses
  ...
  Menus, dcLib, dchook, dcsystem, dcconsts;
...
//    FHint:String;
    ...
    Procedure HideHintControl;
    Procedure HookWndProc(Var Message:TMessage);
    Procedure InternalMouseDown;
//    Procedure OnHintVisibleChanged(Sender:TObject);
    Procedure Resize;
...
Type
  ...
  Private
    FFormsHooks:TList;
    FFormCreateWndInfo:THookInfo;
    FFormCreateWndProcAddr:Pointer;
...
{ Form hooked create and destroy window }

Procedure FormCreateWndProc(Self:TObject);
Type
  TCreateWndProc=Procedure(Self:TObject);
Var
  FormHooks:TFormHooks;
  FormButtons:TFormButtons;
  Hooks:TFormsHooks;
Begin
  Hooks:=FormsHooks;
  RestoreHook(Hooks.FFormCreateWndInfo);
  TCreateWndProc(Hooks.FFormCreateWndProcAddr)(Self);
  Hooks.FFormCreateWndInfo:=SetupHook(Hooks.FFormCreateWndProcAddr,
                                      @FormCreateWndProc);
  If Hooks.IndexOf(TForm(Self))<0 Then
    Exit;
  FormHooks:=Hooks.Forms[TForm(Self)];
  FormButtons:=FormsButtons.Forms[TForm(Self)];
  If FormHooks.FHooks.Count<>0 Then
  Begin
    TFormHook(FormHooks.FHooks[0]).StoreOldWndProc;
    TFormHook(FormHooks.FHooks[FormHooks.FHooks.Count-1]).SetHook;
  End;
  FormButtons.UpdateButtons(0);
End;
...
Begin
  ...
  FNewWndProc:=MakeObjectInstance(WndMethod);
  StoreOldWndProc;
  SetHook;
  ...
  Begin
    ...
    Begin
      If Form.HandleAllocated Then
        SetWindowLong(Form.Handle,GWL_WNDPROC,Integer(FOldWndProc))
    End    
    Else
...
Begin
  With Message Do
    Result:=CallWindowProc(FOldWndProc,FOwner.Form.Handle,Msg,wParam,lParam);
End;
...
Begin
  ...
  Begin
    SetWindowLong(FOwner.Form.Handle,GWL_WNDPROC,Integer(FNewWndProc));
  End;
End;
...
Begin
  FOldWndProc:=Pointer(GetWindowLong(FOwner.Form.Handle,GWL_WNDPROC));
End;
...
Begin
  ...
  FFormCreateWndProcAddr:=GetVirtualProcAddress(TPublicForm,TForm,@TPublicForm.CreateWnd);
  FFormCreateWndInfo:=SetupHook(FFormCreateWndProcAddr,@FormCreateWndProc);
End;
...
Begin
  RestoreHook(FFormCreateWndInfo);
  Clear;
  FFormsHooks.Free;
  Inherited;
End;
...
//  FDCHint:=TDCHint.Create(Self);
//  FDCHint.AutoHide:=True;
//  FDCHint.Immediate:=False;
//  FDCHint.OnVisibleChanged:=OnHintVisibleChanged;
End;
...
Procedure TDCCaptionButton.HideHintControl;
Begin
//  FDCHint.CancelHint;
End;
...
Begin
  ...
  If FShowHint And (PtInRect(FCaptionRect,Point(X,Y))){ And Not FMouseOnControl} Then
    ShowHintControl;
...
{Procedure TDCCaptionButton.OnHintVisibleChanged(Sender:TObject);
Var
  Pt:TPoint;
Begin
  GetCursorPos(Pt);
  Pt:=Point(ConvertXFromScreen(Pt.X),ConvertYFromScreen(Pt.Y));
  FMouseOnControl:=PtInRect(FCaptionRect,Pt);
End;}
...
begin
  ...
  If FForm.HandleAllocated And (GetActiveWindow=FForm.Handle) Then
    FActive:=True
  Else
    FActive:=False;
...
Procedure TDCCaptionButton.ShowHintControl;
Begin
{  If Not FDCHint.Visible And Not (csDesigning In ComponentState) Then
  Begin
    FDCHint.ActivateHint(FHint);
