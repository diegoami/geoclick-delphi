...
interface
uses
  dclib,classes,sysutils,windows,typinfo,buttons,
  dialogs,dcconsts,menus,forms,messages,controls,dcapi,
  dcsystem,dccommon,dcstdctl,comctrls
...
begin
  ...
  with TOpenDialog.Create(nil) do
    try
      Options:=[ofHideReadOnly,ofPathMustExist,ofFileMustExist,ofAllowMultiSelect];
      FilerID:='';
  ...
  begin
    ...
    begin
      ...
      if FNeedSaveAll and FSaveAll then
        ...
        DlgResult:=MessageDlg(Format(SSaveChanges,[IDEObject.DisplayName]),mtConfirmation, AButtons, 0);
      case DlgResult of
        idYes:
...
//BeginSkipConst
const
  VerbArray:Array[0..25] of TVerbData=(
...
//    (Name:'Run.RunParameters';GetState:nil;Execute:RunRunParameters)
...
begin
  ...
  if IDEObject.WasSaved then
    ...
    begin
      ObjectProps:=IDEObject.GetObjectProps;
      {$IFDEF DEBUG}
//      SendDebug(IDEObject.FileName+':'+ObjectProps);
      {$ENDIF}
      SetModifiedTrue;
    end
  else
...
// ---------------- new items registration -------------------
  ...
  RegisterClasses([TIDEObject,TIDEProject]);
...
finalization
  FreeObject(rmanswer);
//  FreeObject(FSystemmessageList);
  FreeObject(FFakeMainObject);
end.
