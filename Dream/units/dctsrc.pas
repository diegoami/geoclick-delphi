...
uses
  ...
  dcconsts, dcstdctl, dctslite, dclib, dchook ;
...
var CE:TComponentEditor;
    D:TFormDesigner;
begin
  if ValidSource and (TObject(ObjID) is TComponent) then
  begin
    D:=GetDesigner(FTreeLink);
    if D = nil then
      D:= GetDesigner(TComponent(ObjId));
    if Assigned(D) then
    begin
