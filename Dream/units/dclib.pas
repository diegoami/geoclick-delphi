...
{$I dc.inc}
uses
  consts, dcconsts, classes, sysutils,
  {$IFDEF WIN32}
  windows, messages,
  {$ELSE}
  dcactivex,
...
procedure FreeList(var list);
procedure FreeSortedList(var List);
...
begin
  ...
  ClearSortedList(TObject(List) as TSortedList);
  FreeObject(TObject(list));
end;
...
begin
  If Source is TDCNamedItem then
    dcLib.AssignAllProps(Source,Self)
  else
    inherited;
end;
...
begin
  ...
  if result = fCapacity then
    SetCapacity(max(fCapacity * 2, 256));
...
begin
  Result:=Nil;
  If (FListDataClass<>TSortedListData) and
    (not FListDataClass.InheritsFrom(TSortedListData)) then
   exit;
