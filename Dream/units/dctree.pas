...
uses
  ...
  ComCtrls,
  dsgnintf,
  CommCtrl,dcgen,menus,dialogs,dcdtree, dcconsts,dclib
...
type
  ...
  TtreeViewOption = (toProcessInsKey,toProcessDelKey,toCanEdit,toCanNavigate,toProcessEnterKey,
                    toProcessDblClick,toTrackInsert,toSmartPopup, toConfirmDelete,toCheckChild{,toShowLeafs});
  ...
  if  Assigned(Node) then
    with Node do
      ...
      begin
        ...
        begin
          S := GetSelfSource(TempItem);
          if (PseudoItem(TempItem)=nil) and ( (Assigned(S) and S.HasChild(TempItem.GetObjId))
                                        or not (toCheckChild in TDCTreeView(Self.Owner).FOptions) ) then
             inherited InternalAdd(TempItem, '', nil, naAddChild);
...
begin
  if Assigned(OnDragOver) then
    DragOver(ASource, X, Y, State, Accept)
  else
    Accept:=(ASource = Self);
end;
