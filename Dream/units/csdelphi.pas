...
{$IFDEF D3}
uses
  dbconsts,classes,dbtables,dbclient,{$IFDEF D5}provider{$ELSE}bdeprov{$ENDIF},dialogs,
  dcconsts,treemod,dcgen,dcsystem, dclib;
{$ENDIF}
...
begin
  ...
  with TComponent(Dset).Owner do
    for i:= 0 to ComponentCount-1 do
      if (Components[i] is TDbDataset) and (Components[i]<>DSet) then
  ...
  if U.Count >0 then
    ...
    begin
      ...
      begin
        ...
        try
          Prov.DataSet:=TDbDataset(Source);
          {$IFNDEF D5}
          Prov.Reset(true);
          {$ENDIF}
          with TClientDataset(Dset) do
