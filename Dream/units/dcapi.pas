...
uses
  ...
  {$IFDEF WIN32}
  windows, {$IFDEF D3}activex, {$ELSE}ole2, {$ENDIF}
  messages,
...
type
  ...
  TDelphiVer = (dvD2,dvC1,dvD3,dvC3,dvD4,dvC4,dvD5,dvC5);

  TDelphiVers=set of TDelphiVer;

const
(* cNativeVerDefine:Array[TDelphiVer] of String=(
   'VER90',   //D2
   'VER93',   //C1
   'VER100',  //D3
   'VER110',  //C3
  ...
  cDelphiVers : array[TDelphiVer] of string = ('D2', 'C1', 'D3', 'C3', 'D4', 'C4', 'D5', 'C5');
  DelphiReqVer:Array[TDelphiVer] Of Integer =(-1{D2},-1{C1},$50053{D3},$30046{C3},$5006C{D4},$E000B{C4},-1{D5},-1);
...
//BeginSkipConst
(*
function  SetupOpenFileQueue : THandle; stdcall; external 'setupapi.dll' name 'SetupOpenFileQueue';
procedure SetupCloseFileQueue(Queue : THandle); stdcall; external 'setupapi.dll' name 'SetupCloseFileQueue';
function  SetupQueueDelete(QueueHandle : THandle; const Path1, Path2 : string) : BOOL; stdcall; external 'setupapi.dll' name 'SetupQueueDeleteA';
function  SetupCommitFileQueue(OwnerWnd : THandle; Queue : THandle; MsgHandler : TPSPFILECALLBACK; Context : pointer) : BOOL; stdcall; external 'setupapi.dll' name 'SetupCommitFileQueue';
function  SetupDefaultQueueCallback(Context : pointer; Notification : UINT; Param1, Param2 : UINT) : UINT; stdcall; external 'setupapi.dll' name 'SetupDefaultQueueCallback';
function  SetupInitDefaultQueueCallback(OwnerWnd : THandle) : pointer; stdcall; external 'setupapi.dll' name 'SetupInitDefaultQueueCallback';
function  SetupQueueCopy(Queue : THandle; const SourceRootPath, SourcePath, SourceFileName, SourceDescription,  SourceTagFile, TargetDirectory, TargetFileName : string; CopyStyle : DWORD) : BOOL; stdcall; external 'setupapi.dll' name 'SetupQueueCopyA';
function  SetupInstallFile(InfHandle : THandle;  InfContext : pointer;  const SourceFile, SourcePathRoot, DestinationName : string; CopyStyle : DWORD; CopyMsgHandler : TPSPFILECALLBACK; Context : pointer) : BOOL; stdcall; external 'setupapi.dll' name 'SetupInstallFileA';
*)
{$IFDEF WIN}
...
const
  ...
  ('delphi.cnt', 'bcb.cnt', 'delphi3.cnt', 'bcb3.cnt', 'delphi4.cnt','bcb4.cnt','delphi5.cnt','bcb5.cnt');
...
begin
 if ver in [dvD2, dvD3, dvD4, dvD5] then
   result := 'delphi32'
 else
   result := 'bcb';
end;
...
Var
  CompFileName:String;
Begin
  DelphiPath:=GetSpecificDelphiDir(DelphiVer);
  CompilerPath:=DelphiPath+'Bin\';
  ...
  Begin
    ...
    _DeleteFile('.dcp');
  End
  Else If CompareText(FileExt,'.bpk')=0 Then
  Begin
    _MoveFile(GetPackageExt(DelphiVer));
    _DeleteFile('.tds');
    _DeleteFile('.obj');
