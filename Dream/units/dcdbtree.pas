...
uses
  ...
  dcsystem, Db, DbCtrls, dctree, dclib, Dctsrc, dctslite, treemod,
...
begin
  ...
  try
    ...
    begin
      BeginDesign;
      try
        AClass := TFieldClass(FindClass('T' + FieldTypeCombo.Text + 'Field' )); //don't resource
        if AClass <> nil then
          with Dataset do
          begin
...
begin
  ...
  (ObjId <> 0) and (DestId <> 0) and (TBookRec(ObjId).ParentID <> TBookRec(DestId).ParentID);
end;
