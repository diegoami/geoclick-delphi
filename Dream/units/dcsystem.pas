...
uses
  ...
  dcclasses, classes, sysutils,
  typinfo, dcapi, dchook,
  {$IFDEF WIN}
...
{$ENDIF}
  ...
  TMethodType = (mtProc, mtMethod, mtClassMethod, mtConstructor, mtDestructor, mtScriptProc);
...
procedure DisposeList(Var AList);
...
begin
  if Pointer(Alist) <> nil then
    with TObject(AList)  as TList do
      begin
        for i := 0 to Count - 1 do
          Dispose(Items[i]);
...
begin
  ...
  Result:=(FileAttrs<>-1) and ((FileAttrs and faReadOnly)<>0);
end;
...
var
  ...
  LogFileName:String;
begin
  LogFileName:=AddSlash(ExtractFilePath(ParamStr(0)))+'DC.LOG';   //don't resource
  ...
  Writeln(F,AppGUID,'  ',DateTimeToStr(Now),'  ',VarToStr(Value));
  Close(F);
end;
{$ELSE}
procedure WriteToLog(const Value:Variant);
...
procedure mregisterproc(const Page: string; ComponentClasses: array of TComponentClass);
begin
...
procedure mregisternoicon(ComponentClasses: array of TComponentClass);
begin
...
procedure mregisternonactivex(ComponentClasses: array of TComponentClass;
...
{$ENDIF}
  ...
  WriteToLog('------------- Application ['+ParamStr(0)+'] started -------------');//don't resource
  {$ENDIF}
finalization
{$IFDEF WIN}
  FreeObject(IDEActiveNotifier);
  DoneGlobalOptions;
  FreeObject(FRegistry);
...
{$ENDIF}
  ...
  FreeObject(FNotifiers);
  FreeObject(FWin32FileSystem);
  FreeStrings(TStrings(FFileSystemList));
  FreeObject(FObjectParamList);
  {$IFDEF LOGFILE}
  WriteToLog('------------- Application ['+ParamStr(0)+'] closed -------------');//don't resource
  {$ENDIF}
end.
