...
uses
  ...
  Buttons, StdCtrls,
  typinfo, extctrls, comctrls, commctrl, dccommon, dclib, dccontrols,
  shellapi, dcconsts{$ifdef D3},extdlgs{$endif}{$IFDEF USEMASKEDIT},mask{$ENDIF};
...
type
  ...
  public
    ...
    function    Focused : boolean; {$IFDEF D4} override; {$ENDIF}
    procedure   Deselect;
    procedure   SelectAll;
...
{$ENDIF}
  ...
  private
    ...
    function  CalcSize(const S : string) : integer;
...
function PopupListbox(AParent : TWinControl; Sl : TStrings ; X, Y : integer;
          ...
          AFont : TFont; ACallBack : TPopupListBoxCallback):TDCPopupListBox;
  ...
  with Canvas do
    if TCustomDCEdit(Owner).IsFlat then
      begin
        ...
        if (TCustomDCEdit(Owner).IsSingleBorder) then begin 
...
begin
  ...
  with Params do
    begin
      ...
      if NewStyleControls and not Flat and Ctl3D and (fBorderStyle = bsSingle) then
        begin
...
procedure TCustomDCEdit.CreateHandle;
begin
  inherited;
//  if (fDropWindow <> nil) and (fDropWindow.HandleAllocated) then
  ...
  with Canvas, R, Pen do
    if (csDesigning in ComponentState) or
      ...
      begin
        ...
        OffsetRect(R, -1, -1);
        if Is3DLine and UseDreamBorder then begin
        ...
        else if IsSingleBorder then begin
        ...
        begin
          ...
          MoveTo(Left, Bottom);
          LineTo(Right, Bottom);
          if (DreamBorderStyle = dbsLineBorder) then begin
            LineTo(Right, Top);
            LineTo(Left, Top);
            LineTo(Left, Bottom);
          end;
        end
        else if not (UseDreamBorder and (DreamBorderStyle = dbsNone)) then begin
          Color := GetBottomLineColor;
          MoveTo(Left + 1, Bottom);
          LineTo(Right, Bottom);
          LineTo(Right, Top - 1);
...
begin
  ...
  if (fButton2.Visible) and (x >= tl2.x) then
    ...
    if fButton1.Visible and (x > tl.x) then
...
begin
  Result := IsEditAvailable and LongBool(Edit.Perform(CB_GETDROPPEDSTATE, 0, 0));
end;
...
begin
  ...
  SetRect(Loc, 1, ydelta + 1, ClientWidth - delta - ydelta * 2 - 2, ClientHeight - 2 - ydelta * 2);
    ...
    begin
      ...
      if fEdit is TWinControl then
        begin
          ...
          TMWinControl(fEdit).TabStop := false;
        end;
...
begin
  ...
  with Canvas do
    begin
      ...
      if fDisplayNames then
        begin
          StrPCopy(Text, Copy(ColorToString(TColor(Items.Objects[Index])), 3, 40));
...
begin
  ...
  if index <> -1 then
    begin
      ...
      StrPCopy(pchar(Msg.LParam), ColorToString(fColorValue));
    end
  else
    pchar(Msg.LParam)^ := #0;
end;
...
begin
  ...
  for i := 0 to ColorsInList - 1 do
    if TCustomDCEdit(Owner).Text = ColorToString(TColor(Items.Objects[i])) then
      begin
        ItemIndex := i;
        break;
      end;
...
begin
  ...
  for i := penmin to penmax do begin
...
begin
  ...
  begin
    ...
    begin
      ...
      DrawWithSelection(Canvas.Handle, Font, FBoldFont, Rect, s , BPos, BLen, FAlign);
...
function TDCListBox.CalcSize(const S : string) : integer;
begin
  result := CalcStringSize(Canvas.Handle, Font ,FBoldFont, S + ' ').cx ;
end;
    ...
    begin
      ...
      begin
        if Cnt = FInternalList.Count then
          FInternalList.Add('');
        size := CalcSize(ExtractColumn(Items[i], Pos));
...
function PopupListbox(AParent : TWinControl; Sl : TStrings ; X, Y : integer;
          AlignArray : Array of TLeftRightAlign;
          AFont : TFont;
          ACallBack : TPopupListBoxCallback) : TDCPopupListBox;
var
  p         : TPoint;
  i         : integer;
begin
