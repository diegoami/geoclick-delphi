...
uses
  windows, messages, classes, dcgen, dsgnintf, controls, dcsystem, dclib,
...
//    flistknown  : boolean;
    ...
    fpedit          : TPropertyEditor;
    fSetEditor      : TPropertyEditor;
    ...
    procedure FillList;
    procedure SetPropEditor(val : TPropertyEditor);
    procedure FreePropEditor;
    procedure SetLinkedEditor(val : TDCCustomPropEditor);
    function  GetPropertyEditor(const propname : string) : TPropertyEditor; virtual;
  ...
  public
    ...
    property  PropEditor : TPropertyEditor read fPropEdit write SetPropEditor;
...
{$ENDIF}
 ...
 TPictureEdit = class(TGraphEdit)
 protected
   function GetGraphic : TGraphic; override;
 end;

 TImageEdit = class(TGraphEdit)
 protected
   function GetGraphic : TGraphic; override;
 end;
...
type
  TMultiSelectPropEdit = class(TPropertyEditor)
  private
    function GetPropEdit(index : integer) : TPropertyEditor;
  public
    PropEditList : TList;
    ...
    property   PropEdit[Index: Integer] : TPropertyEditor read GetPropEdit;
  end;
...
function GetPopupClass (PropEdit : TPropertyEditor) : TWinControlClass;

function GetEditClass (PropEdit : TPropertyEditor) : TControlClass;
...
begin
  result := TPicture(TMPropEditor(TDCCustomPropEditor(Owner).PropEditor).GetOrdValue).Graphic;
end;
...
begin
  result := TGraphic(TMPropEditor(TDCCustomPropEditor(Owner).PropEditor).GetOrdValue);
end;
    ...
    with TPopupListBox(DropWindow) do
      begin
        ...
        if Items.Count = 0 then
          FillList;
        if fPropEdit <> nil then
          ItemIndex := Items.IndexOf(fPropEdit.Value);
...
procedure TDCCustomPropEditor.GetPropEdit(Prop: TPropertyEditor);
var
  sname : string;
  ppos  : integer;
begin
...
function TDCCustomPropEditor.GetPropertyEditor(const propname : string) : TPropertyEditor;
var
...
procedure TDCCustomPropEditor.SetPropEditor(val : TPropertyEditor);
begin
...
begin
  result := GetPopupClass(PropEditor);
end;
...
begin
  with PropEdit do
    if PropEdit is TSetElementProperty then
      begin
      ...
      begin
        if PropEdit is TMultiSelectPropEdit then
          proptype := TMultiSelectPropEdit(PropEdit).GetRealPropType
        else
...
var
  i : integer;
begin
  result := PropEdit[0].GetEditLimit;
  for i := 0 to PropEditList.Count - 1 do
    result := min(result, PropEdit[i].GetEditLimit);
end;
...
function TMultiSelectPropEdit.GetPropEdit (index : integer) : TPropertyEditor;
begin
 result := TPropertyEditor(PropEditList[index]);
end;
